...
When changed to `yes`, PRs run through the same labels and states as issues, using the corresponding `gh pr` commands.
...
- **Child ticket**: a GitHub sub-issue linked to the map. If sub-issues are unavailable, add it to a task list and include `Part of #<map>` in the child.
- **Blocking**: use GitHub native issue dependencies. If unavailable, use a `Blocked by: #<n>` line.
- **Frontier query**: select the first open child without open blockers or an assignee.
